...
void motionCapture::uniteContours(std::vector<std::vector<cv::Point>> &cnts) {
    bool isCrossed = true;
    std::vector<cv::Point> stub = {cv::Point(1, 1)};
    while (isCrossed) {
        isCrossed = false;
...
void motionCapture::find(Mat &gray) {
    ...
    if (!allContours.empty() && allContours.size() < 1000) {
        ...
        if (m_allTracks.empty()) {
            ...
            std::multimap<int, int> pointsNum;
            std::vector<Point2f> pointsPrev, pointsNow;
            long trackNumber = 0;
            for (const auto &track : m_allTracks) {
                if (!track.empty()) {
                    std::vector<Point2f> tmpVec;
            ...
            calcOpticalFlowPyrLK(m_prevGray, gray, pointsPrev, pointsNow, status, err, m_winSize, 3, m_criteria, 0,
                                 0.001);
            trackNumber = 0;
            for (auto &track : m_allTracks) {
                if (!track.empty()) {
                    std::vector<Point2f> tmpVecPoints;
                    const auto &point_iter = pointsNum.equal_range(static_cast<const int>(trackNumber));
                    for_each(point_iter.first, point_iter.second, [&tmpVecPoints, &pointsNow](const auto &it) {
...
void motionCapture::fill_tracks(std::vector<std::map<milliseconds, std::vector<Point>>> &allTracks,
                                std::vector<std::vector<Point>> &allContours) const {
    milliseconds cur_time = m_currentTime;
    for_each(allContours.begin(), allContours.end(), [&allTracks, &cur_time](const std::vector<Point> &cnt) {
        std::map<milliseconds, std::vector<Point>> oneTrack;
